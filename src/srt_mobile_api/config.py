...
통과한 값은 표준형으로 다시 쓰입니다. :mod:`srt_mobile_api.safety` 의 경로 검사가
그 표준형을 전제로 하므로 이 클라이언트를 임의의 호스트로 돌릴 방법은 없습니다.
"""
...
class SrtConfig:
    ...
    기본값 그대로 쓰면 됩니다. 전부 앱 v2.0.41 이 쓰는 값이고, 특히
    ``user_agent`` 끝의 ``SRT-APP-Android V.2.0.41`` 은 서버가 앱 요청으로 인정하는
    표식이라 지우면 안 됩니다.

    **오리진은 고정입니다.** ``base_url`` 과 ``netfunnel_url`` 은 각각
    ``https://app.srail.or.kr`` 와 ``https://nf.letskorail.com`` 이어야 합니다. 스킴이
    HTTPS 가 아니거나, 호스트가 다르거나, 443 아닌 포트·경로·질의·프래그먼트·
    사용자정보가 붙어 있으면 :class:`ValueError` 입니다. 통과한 값은 표준형 상수로
    다시 쓰이므로 이 클라이언트를 임의의 서버에 겨눌 수 없습니다.
    :mod:`~srt_mobile_api.safety` 의 경로 검사가 이 표준형을 전제로 합니다.

    ``device_key`` 는 비어 있으면 안 되고 ``timeout`` 은 양수여야 합니다.

    ``live_env_var`` 는 라이브 실행을 켜는 환경변수의 **이름**일 뿐이라 바꿔도 게이트가
    따라오지 않습니다 — :mod:`~srt_mobile_api.live` 는 ``SRT_MOBILE_API_LIVE`` 라는
    이름을 직접 읽습니다.
    """
