...
문서 정정과 실서버 재검증, 그리고 복구 도구의 안내 개선입니다. 공개 API 는 그대로입니다.

### Fixed

- **환승 예약이 "한 번도 보낸 적 없습니다" 라고 잘못 적혀 있었습니다.**
  `reserve_transfer` 와 `TransferItinerary` 의 docstring, `docs/VERIFICATION.md`,
  `docs/IMPLEMENTATION_PROGRESS.md` 의 제목이 모두 그렇게 말했지만 같은 progress 파일이
  2026-07-26 동대구→광주송정 성공 기록을 담고 있었습니다 — 파일이 스스로 모순됐습니다.
  이제 예약·취소는 확인, **결제는 미확인** 이라고 적습니다.
- **`recover_hold.py` 의 실패 안내가 쓸모없었습니다.** 취소가 거절되면 "이 명령을 다시
  실행하거나 앱에서 취소하세요" 라고만 했는데, `ERR800052` 는 결정적인 거절이라 같은
  명령은 영원히 같은 결과를 냅니다. 이제 그 코드가 오면 원인(여정 수 불일치)을 짚고
  PNR 이 채워진 `--journey-count 2` 명령을 그대로 보여 줍니다. 이미 `2` 로 보낸
  경우에는 다시 권하지 않습니다.
- **`docs-deploy.yml` 의 첫 이유가 낡았습니다.** 저장소가 비공개라는 설명이었는데 공개
  됐고 그 워크플로로 이미 배포했습니다. `push:` 트리거가 없는 진짜 이유만 남겼습니다.

### Verified

- **환승 예약 재확인.** 동대구(0015)→목포(0041), 열차 304 + 653, 오송 환승, 성인 1명 →
  `SUCC`/`IRR000018`. 유추(inferred) 등급이던 슬롯 2 키 5개가 다시 받아들여졌습니다.
- **취소는 `jrnyCnt="2"` 가 필요합니다.** 그 홀드를 PNR 문자열만으로 취소하니
  `FAIL`/`ERR800052` 로 세 번 거절당했고, `journey_count="2"` 로는 첫 시도에
  `SUCC`/`IRG000000` 였습니다. 홀드 객체를 넘기면 자기 값을 기억하므로 저절로 맞습니다.
- **환승 조회에는 코레일 열차가 섞여 옵니다.** 부산→익산 5개 중 3개가 구간 하나 이상
  KTX(`stlbTrnClsfCd="00"`) 또는 KTX-산천(`"07"`)이었고, 넘기면
  `personal_reservation_payload` 가 전송 전에 거절합니다. 남단 교차 20개 조합을 훑어
  양쪽이 다 `"17"` 인 여정이 24개 있음을 확인했습니다 — 예약 가능한 것은 그것뿐입니다.
- **역은 코드로 넘겨야 합니다.** 한글 역 이름을 주면 조회가
  `WRG200004 입력값오류(출발역,도착역,경유역,지정역)` 로 거절합니다. KORAIL 쪽은 이름을
  받아 주므로 두 패키지를 같이 쓰면 걸리기 쉬운 차이입니다.
- 환승 **결제** 는 여전히 미확인입니다.

## 1.1.0 - 2026-07-31

APK 디컴파일 산출물과 코드를 다시 대조한 감사 한 바퀴입니다. 공개 API 는 그대로고,
빨간 상태였던 오프라인 게이트가 고쳐졌고, 소스에서 약 1,900줄이 빠졌습니다.

### Fixed

- **오프라인 게이트가 실패하고 있던 것.**
  `test_repository_truth_and_full_mutation_policy` 가
  `TypeError: expected string or bytes-like object, got 'NoneType'` 로 실패했는데,
  개수가 어긋난 것처럼 읽히지만 아닙니다. `_collected_offline_test_count` 가 자식
  pytest 에게 자기 개수를 물을 때, 자식은 stdout 이 파이프면 인코딩을 로케일에서
  가져오므로 한국어 Windows 에서 cp949 로 씁니다. 이 스위트에는
  `test_three_person_minimum_belongs_to_exactly_다자녀_and_3세대` 가 있어서
  `--collect-only` 출력이 cp949 바이트가 되고, 부모가 `encoding="utf-8"` 로 읽다가
  `subprocess` 의 reader 스레드가 0xb4 에서 죽고 `result.stdout` 이 `None` 이
  됩니다. 자식 환경에 `PYTHONIOENCODING=utf-8` 을 못박아 발생 지점에서 고쳤습니다.
  부모에 `errors="replace"` 를 주면 크래시는 멈추지만 깨진 글자를 통과시켜 놓고
  고쳐진 척하므로, 어긋남을 알아채는 것이 일인 검사에서는 더 나쁩니다.

### Changed

- **소스 15개 모듈에서 1,931줄이 빠졌습니다** (2,595줄 삭제 / 664줄 추가). 동작 변경은
  없습니다 — 공개 표면, 라우트, 필드, 봉투 처리가 모두 그대로입니다. 같은 규칙을
  반복하던 설명, 코드를 변호하던 주석, 이전 판이 무엇을 했는지 돌아보던 서술을
  지웠습니다. `errors.py` 는 예외 이름이 이미 말하는 것을 한 단락씩 설명하던
  docstring 때문에 절반이 줄었고, `netfunnel.py` 와 `safety.py` 는 20~70줄 주석
  블록이 네 줄로 같은 사실을 담습니다.
  **라우트 인용과 안전 게이트는 하나도 지우지 않았습니다** — 허용목록 항목과 본문
  기반 카드 비밀값 검사를 포함해서입니다. "v2.0.41 에서 0-hit", "srtgo 유래" 같은
  근거 표기도 전부 남겼습니다. 이 패키지의 라우트 10개는 번들 근거가 없고 그렇다고
  적혀 있으며, 산문을 줄이는 것이 추측을 조용히 사실로 승격시킬 이유는 아닙니다.

### Verified

이 절은 코드 변경이 아니라 **이번에 실서버가 답한 것** 입니다.

- **예약→취소 왕복 3회차.** 2026-07-31 이 07-25, 07-26 에 이어지고 세 번 모두 같은
  봉투를 돌려줍니다: reserve `SUCC`/`IRR000018`, cancel `SUCC`/`IRG000000`, 승차권
  목록에 흔적 없음. 수서→부산, 2026-08-10, 열차 341, 성인 1명, 일반실, 즉시 해제.
  리팩토링 직후에 돌린 이유는, 오프라인 테스트만이 지키는 와이어 형태는 아무도 확인
  하지 않은 와이어 형태이기 때문입니다 — 오프라인 스위트는 올바른 폼과 서버가 거절할
  폼을 구별하지 못합니다.
- **막아서 아무것도 보내지 않은 두 경우.** 06:00 출발로 조회한 열차 10개가 전부
  `좌석매진` 이어서 스크립트가 전송 전에 거부했고 홀드는 만들어지지 않았습니다 —
  실패가 아니라 동작입니다. 그리고 여석이 있던 13:55 열차 하나는
  `personal_reservation_payload` 가
  `reservation requires an SRT train (service_class_code '17')` 로 막았습니다. SRT
  조회 결과에 섞여 온 코레일 열차를 폼 조립 단계에서 걸러낸 것입니다.
- **APK 대조.** 이 앱은 WebView 껍데기라 근거가 java `@Field` 가 아니라
  `apktool/assets/` 의 `ara1001l.js`·`netfunnel.js`·`ticketList.html`·`sub/main.html`
  에 있습니다. 인용 43건을 확인했고 정정 0건, 와이어 포맷 불일치 0건입니다. 이미
  fixture 로 고정돼 있던 패키지에서 기대할 수 있는 결과이며, 아무것도 들여다보지
  않았다고 오해되지 않도록 결과로 남깁니다.
