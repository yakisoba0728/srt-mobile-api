...
This file is the audit log of the reverse-engineering effort behind
`srt-mobile-api`. It records what was observed, when, against what, and what
each observation does **not** establish. It used to be the first four hundred
lines of the README; it was moved here so the README could serve someone who
wants to use the library, and not one word of it was dropped.

Read it when you need to know how much weight a claim carries — which routes a
live run has actually answered, which field names rest on a single third-party
source, which shapes are inferred from the offline app bundle and have never
been sent. [README.md](../README.md) tells you what the library does;
this file tells you why any of it should be believed.

Three conventions run through the whole document:

- **live-verified** means a real request was sent to `app.srail.or.kr` and the
  response was recorded. The date, the server's own `msgCd` and the exact scope
  of the run are always stated, because one round trip is not a general proof.
- **bundle-evidenced** means the shape comes from the decompiled SRT Android
  app v2.0.41 (21,673 files) — usually its JavaScript — and has not been sent.
- **srtgo-attested** means the shape comes from the third-party `srtgo` client's
  live runs and is **0-hit** in our own bundle. That is real evidence of a
  different kind, and it is never silently upgraded.

Corrections are made **in place and marked**, never deleted. Several claims in
this document were disproved by a later run; each one is kept alongside its
refutation so a future reader does not re-inherit a fear or a hope that was
already settled.

## Verification status at a glance

Each mutation CATEGORY is live-enabled because a live run answered its own wire
...
produced the live-response findings recorded through this document: the
...
in [IMPLEMENTATION_PROGRESS.md](IMPLEMENTATION_PROGRESS.md).
...
[IMPLEMENTATION_PROGRESS.md](IMPLEMENTATION_PROGRESS.md) under
"할인 / 쿠폰 / 공공할인 — the survey".
