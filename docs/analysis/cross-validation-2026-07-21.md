...
13. **Which of the two Google API keys** (`<SRT-APP-GOOGLE-API-KEY-REDACTED>` at `strings.xml:171,174` vs `<SRT-APP-GCM-API-KEY-REDACTED>` at `strings.xml:492`) is live/current for FCM vs Maps/crash? Both Android-restricted, low severity. *(This line carried a truncated prefix of each real key until 2026-07-27; the prefixes identified which key was meant, which is what the placeholder names now do without reproducing any of the value.)*
