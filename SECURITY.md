...
"cancel"}` — the two halves of one reversible operation. Each of the two also
binds the target route to the caller's category (`assert_mutation_route` plus
`assert_mutation_route_category`), so an enabled category cannot be aimed at
another category's endpoint. **The `payment` and `refund` routes therefore
cannot be reached even through the low-level client** — neither by naming their
category nor by pointing an enabled category at their path — and a payment
carries a further `fake_card_only` gate behind that. A security report must not
attempt to defeat those gates, exercise an excluded mutation, or make an
unapproved production request.
