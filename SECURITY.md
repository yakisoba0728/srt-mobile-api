...
This package intentionally transmits no state-changing request. Payment, refund,
and seat holding or selection have no client method at all. Reservation has a
...
previews by default; asked to send, it is refused by the gate below, and its
wire shape is srtgo-attested and unconfirmed against our app version. The
mutation send path itself (`post_mutation_form`, and the underlying
`_send_mutation_request`) refuses
